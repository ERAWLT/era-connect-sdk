...
| Bitcoin messages | Legacy P2PKH only; any segwit address returns an EMPTY signature. The reply bytes are the **ASCII of a base64 string**, not raw bytes | typed `empty-signature`; double-decode in `parse()` |
